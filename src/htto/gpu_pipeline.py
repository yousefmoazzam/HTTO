...
def __option_parser():
    ...
    parser.add_argument(
        "-nc", "--ncore", type=int, default=0, help="The number of cores."
    )
    parser.add_argument(
        "-pad",
        "--pad",
        type=int,
        default=0,
        help="Number of slices to pad each block of data.",
    )
    parser.add_argument(
        "-m",
        "--methods",
        type=int,
        default=1,
        help="A method component to add: 1 - median filter, 2 - (1) + normalisation, 3 - (2) + stripes removal, 4 - (3) + centering, 5 - (4) + reconstruction",
    )
...
def main():
    ...
    for i in range(args.repeat):
        ...
        data = load_h5.load_data(
            args.in_file, dim, args.path, preview=preview, pad=pad_values, comm=comm
        )
        ...
        print(
            f"Process {rank}'s data shape is {(angles_total, detector_y, detector_x)} of type {data.dtype}"
        )
        ##########################################################################################################
        ...
        if rank == mid_rank:
            ...
            rot_center = tomopy.find_center_vo(
                data[:, mid_slice, :], step=0.5, ncore=args.ncore
            )
        ...
        out_folder = (
            f"{abs_out_folder}/{datetime.now().strftime('%d-%m-%Y_%H_%M_%S')}_recon"
        )
        if rank == 0:
        ...
        if args.dimension == 1:
            ...
            data = load_h5.load_data(
                f"{out_folder}/intermediate.h5", slicing_dim, "/data", comm=comm
            )
            dim = slicing_dim
        ...
        chunk_h5.save_dataset(
            out_folder, "reconstruction.h5", recon, dim, chunks_recon, comm=comm
        )
...
def __send_big(data, dest, comm=MPI.COMM_WORLD):
    ...
    print(
        f"Rank {comm.rank}: sending {n_bytes} bytes in {n_blocks} blocks to rank {dest}."
    )
